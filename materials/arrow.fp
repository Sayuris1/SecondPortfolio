...
varying highp vec3 local_position;
...
void main() {
    ...
    for (float p = meta_pixels_count; p < meta_pixels_count + lights_count * light_pixels_count; p = p + light_pixels_count) {
        ...
        float light_attenuation = pow(max(1.0 - light_distance / light_radius, 0.0), 2.0 * light_smoothness);
        ...
        if (light_5.r < 1.0) {
            ...
            if (light_smoothness > 0.0) {
                ...
                float spot_intensity = max((spot_cutoff - spot_theta) / spot_epsilon, 0.0);
